...
Agents are powerful on their own, but **handoffs** allow you to chain them into a collaborative system that solves complex tasks—this is what we call a [Multi-Agent System (MAS)](https://docs.lookfor.ai/guides/multi-agent-systems).

In simple terms, a _handoff_ is when one agent delegates a task to another. This could happen when:
...
> If you hand off to an agent named `Refund Agent`, the LLM sees a tool named `transfer_to_refund_agent`. This is why you can force a handoff by setting `Tool Choice` in [Model Settings](https://docs.lookfor.ai/guides/model-settings) to `required`.
